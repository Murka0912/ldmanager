...
def connecttosql(subd, db, username, password): #connect to sql server
    server = 'tcp:'+subd
    database = db
    cnxn = sql.connect('DRIVER={SQL Server};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+password)
    cursor = cnxn.cursor()
    return cursor
def dbname(cur): #select dbname
    cur.execute("""SELECT name 
FROM sys.databases""")
    row = cur.fetchall()
    return ([i for i in row])

def indexfrag(cur): #index fragmentation
...
def loadqueries(cur):# top queries
    cur.execute('''SELECT TOP 10
...
def status(cur):# Что происходит на сервере
    cur.execute('''select session_id, status, wait_type, command, last_wait_type, percent_complete, qt.text, total_elapsed_time/1000 as [total_elapsed_time, сек],
...
def indexcount(cur): #количество отсутствующих индексов
    cur.execute('''SELECT [DatabaseName] = DB_NAME(database_id),
...
def non_index_load(cur):# Отсутствующие индексы, вызывающие издержки
    cur.execute('''SELECT TOP 10 
...
def not_used_index(cur): #Неиспользуемые индексы
    cur.execute("""SELECT DatabaseName = DB_NAME(),
       TableName = OBJECT_NAME(s.[object_id]),
       IndexName = i.name,
       user_updates,
       system_updates,
...
def in_out_queries(cur):#Запросы с высокими издержками на ввод-вывод
    cur.execute("""SELECT TOP 10
...
def cpu_used_queries(cur):#Запросы с высоким использованием ресурсов ЦП
    cur.execute("""SELECT TOP 10
       [Average CPU used] = total_worker_time / qs.execution_count,
       [Total CPU used] = total_worker_time,
       [Execution count] = qs.execution_count,
...
def blocked_queries(cur):#Запросы, страдающие от блокировки
    cur.execute("""SELECT TOP 10
...
def in_out_load(cur):#нагрузку на подсистему ввода-вывода
    cur.execute("""select top 5 
...
def who_cpu(cur):#какой процессор что делает
    cur.execute("""SELECT DB_NAME(ISNULL(s.dbid,1)) AS [Имя базы данных],
...
def control(cur):#контроль "несжатости"
    cur.execute("""SELECT tbl.name,
       i.name,
...
def operation_statistic(cur): #статистика по операциям в БД
    cur.execute("""SELECT t.name AS [TableName],
...
def update_date_stat(cur): #дата обновления статиститки
    cur.execute("""SELECT STATS_DATE(t1.object_id, stats_id), 'UPDATE STATISTICS [' + object_name(t1.object_id) + ']([' + t1.name + ']) WITH FULLSCAN',
       i1.rows
  ...
  order by STATS_DATE(t1.object_id, stats_id) DESC
""")
    row = cur.fetchall()
    return ([i for i in row])

def in_out_file_load(cur):#i/o-нагрузка на файлы
    cur.execute("""SELECT TOP 10 DB_NAME(saf.dbid) AS [База данных],
...
def in_out_disk_load(cur):#-- i/o-нагрузка на диски
    cur.execute("""SELECT SUBSTRING(saf.physical_name, 1, 1)    AS [Диск],
...
def disk_usage(cur):#Занимаемое место на диске
    cur.execute("""SELECT TOP 100
...
def memory_objects(cur):#под какие объекты выделена память
    cur.execute("""select count(*)as cached_pages_count,
...
def call_proc(cur, userid,annotation, logPacketId):
    params = (1000,"test call",2)
    cur.execute("""set nocount on; exec GRK_SP_100_HAKATON_REGLOGPACKET 1000,'test3',4""")
    row = cur.commit()
    return row
